...
function showDialogPlayer() {
	...
	if(zorbSpeed != 0) {
		GUILayout.Space(10);
		if(GUILayout.Button((!Game.PlayerVeh.zorbBall ? "Activate" : "Deactivate") + " My Xorb (/x)")) Game.Player.GetComponent.<NetworkView>().RPC("sZ", RPCMode.All, !Game.PlayerVeh.zorbBall);
	}
...
function showDialogPlayers () {
	...
	if(Game.Controller.isHost && Game.Controller.unauthPlayers.length > 0) {
		...
		for(i = 0; i < Game.Controller.unauthPlayers.length; i++) {
			...
			if(GUILayout.Button("Evict")) {
				GetComponent.<NetworkView>().RPC("dN", Game.Controller.unauthPlayers[i].p, 2);
				if(Network.isServer) Network.CloseConnection(Game.Controller.unauthPlayers[i].p, true);
				else GetComponent.<NetworkView>().RPC("cC", RPCMode.Server, Game.Controller.unauthPlayers[i].p, Game.Controller.unauthPlayers[i].n, 0);
			}
				...
				else GetComponent.<NetworkView>().RPC("pI", RPCMode.Server, Game.Controller.unauthPlayers[i].p, Game.Controller.unauthPlayers[i].n);
			}
			GUILayout.EndHorizontal();
		}
	...
	for(var plrE : DictionaryEntry in Game.Players) {
		...
		GUILayout.TextArea(
			(go.GetComponent.<NetworkView>().isMine ? "" : (vehNet ? Mathf.RoundToInt(vehNet.ping * 1000) + " png - " + Mathf.RoundToInt(vehNet.jitter * 1000) + " jtr" + veh.netCode + "\n" : "")) +
			(vehNet && veh.networkMode == 2 ? Mathf.RoundToInt(vehNet.calcPing) + " CalcPng - " + Mathf.RoundToInt(vehNet.rpcPing) + " TmstmpOfst\n" : "" ) +
			(Network.isServer ? go.GetComponent.<NetworkView>().owner.externalIP + " " + go.GetComponent.<NetworkView>().owner.ipAddress : "") +
			"\n" + go.GetComponent.<NetworkView>().viewID.ToString() + " " + veh.networkMode /*+ "/" + go.networkView.owner.ToString()*/
		, GUILayout.Height(30));
		GUILayout.BeginHorizontal();
		if((Game.Controller.isHost || isAdmin) && !veh.isBot && !go.GetComponent.<NetworkView>().isMine && GUILayout.Button("Evict")) {
			Game.Messaging.broadcast(go.name + " was evicted by " + Game.Player.name);
			if(Network.isServer) {
				GetComponent.<NetworkView>().RPC("dN", plrE.Value.networkView.owner, 2);
				plrE.Value.networkView.RPC("dN", RPCMode.All, 2);
				Network.CloseConnection(plrE.Value.networkView.owner, true);
			}
			else {
				GetComponent.<NetworkView>().RPC("cC", RPCMode.Server, plrE.Value.networkView.owner, plrE.Key, 1);
			}
		}
		else if((Game.Controller.isHost || isAdmin) && !veh.isBot && !go.GetComponent.<NetworkView>().isMine && GUILayout.Button("Ban")) {
			...
			if(Network.isServer) {
				...
				GetComponent.<NetworkView>().RPC("cC", RPCMode.Server, plrE.Value.networkView.owner, plrE.Key, 2);
			}
		}
		GUILayout.EndHorizontal();
		GUILayout.FlexibleSpace();
...
function showDialogServer() {
	if((Game.Controller.isHost || isAdmin)) {
		if(GUILayout.Button(">> Default All <<")) {
			Game.Controller.GetComponent.<NetworkView>().RPC("sSS", RPCMode.All, serverDefault);
		}
	...
	if(Game.Controller.isHost) {
		if(GUILayout.Button("Apply Custom\nSettings")) {
			Game.Controller.GetComponent.<NetworkView>().RPC("sSS", RPCMode.All, serverString);
		}
	}
}
...
function updatePrefs() {
	...
	*/
	...
	GameObject.Find("MiniMap").GetComponent.<Camera>().enabled = (minimapAllowed ? useMinimap : false);
...
function saveVehicleColor() {
	...
	Game.Player.GetComponent.<NetworkView>().RPC("sC", RPCMode.Others, Game.PlayerVeh.vehicleColor.r, Game.PlayerVeh.vehicleColor.g, Game.PlayerVeh.vehicleColor.b, Game.PlayerVeh.vehicleAccent.r, Game.PlayerVeh.vehicleAccent.g, Game.PlayerVeh.vehicleAccent.b);
}
