...
function OnGUI () {
	...
	if(QualitySettings.GetQualityLevel() < 3) QualitySettings.SetQualityLevel(3); //Server List GUI looks terrible at 1/4 res :)
...
function TestConnection(force : boolean) {
    ...
    //NAT punchthrough unsupported API
...
function authenticateUser() {
	...
	yield www;
	if(!www || www.text == "") userAuthenticating = "Authentication server is unreachable";
	else if(www.text == "-1") userAuthenticating = "";
	else if(www.text == "-2") userAuthenticating = "Username not found";
	else if(www.text == "-3") userAuthenticating = "Incorrect password";
	else if(www.text == "-4") userAuthenticating = "Too many login attempts";
	else {
		var data = www.text.Split(":"[ 0 ]);
